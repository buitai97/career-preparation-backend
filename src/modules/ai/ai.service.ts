...
import { AIFeedback, AIFeedbackSchema, AIResumeFeedback, AIResumeFeedbackSchema } from "./ai.type";
        ...
        messages: [
            {
                role: "system",
                content:
                    "You are an expert resume reviewer and career coach. Analyze resumes and provide actionable, structured feedback.",
            },
            {
                role: "user",
                content: `Analyze this resume and return JSON feedback.
...
Return JSON in this exact format:
{
  ...
  "sectionTips": {
    "EXPERIENCE": "specific tip for experience section",
    "SKILLS": "specific tip for skills section",
    "EDUCATION": "specific tip for education section"
  }
}`,
            },
        ],
    ...
    const content = response.choices[0].message.content;
    if (!content) {
        throw new Error("Empty response from AI resume feedback model");
    }

    const jsonText = content.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, "").trim();
    const raw = JSON.parse(jsonText);
    return AIResumeFeedbackSchema.parse(raw);
};
